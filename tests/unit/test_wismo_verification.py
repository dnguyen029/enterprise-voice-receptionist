...
MOCK_HEADERS = [
    'System', 'Channel', 'Warehouse', 'Carrier', 'Order Number',
    'PO/Reference', 'Order Date', 'Order Type', 'Status (Order)',
    'Posted Sales Inv.', 'WSH Number', 'Label Status', 'Tracking Number',
    'Shipped Date', 'Postal Code', 'Email', 'Phone', 'State/ Province Code',
    'City', 'Contact Person', 'Customer Order number'
]

MOCK_ROW_1 = [
    'Shopify', 'Online', 'LA', 'FedEx', '10024',
    'PO-10024', '2026-07-01', 'Standard', 'Released',
    '', '', '', '1Z12345',
    '2026-07-02', '92701', 'john@example.com', '+17145550199', 'CA',
    'Santa Ana', 'John Doe', 'PO-10024'
]

MOCK_ROW_2 = [
    'Shopify', 'Online', 'LA', 'UPS', '10025',
    'PO-10025', '2026-07-02', 'Standard', 'Released',
    '', '', '', '1Z67890',
    '2026-07-03', '90210', 'jane@example.com', '+13105550100', 'CA',
    'Los Angeles', 'Jane Smith', 'PO-10025'
]
...
def test_wismo_lookup_caller_id_match_and_zip_verified(mock_auth, mock_sheets_api):
    ...
    with patch.dict("os.environ", {"WISMO_SPREADSHEET_ID": "mock_sheet_id"}):
        ...
        res_verified = wismo_lookup(purchase_order=res_phone["purchase_order"], zip_code="92701")
...
def test_wismo_lookup_po_in_column_u(mock_auth):
    ...
    with patch("app.tools_lib.sheets.requests.get") as mock_get:
        ...
        data_response.status_code = 200
        data_response.json.return_value = {
            "values": [custom_headers, custom_row]
        }
        
        mock_get.side_effect = lambda url, *args, **kwargs: meta_response if "fields=sheets.properties" in url else data_response
...
def test_wismo_lookup_phone_multiple_matches(mock_auth):
    ...
    custom_row_2 = [
        'Shopify', 'Online', 'LA', 'UPS', '10026',
        'PO-10026', '2026-07-03', 'Standard', 'Released',
        '', '', '', '1Z99988',
        '2026-07-04', '92705', 'jane_diff@example.com', '+17145550199', 'CA',
        'Tustin', 'John Smith', 'PO-10026'
    ]
