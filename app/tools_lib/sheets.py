...
def execute_with_retry(method, url, headers, params=None, json_data=None, max_attempts=3) -> requests.Response:
    """Executes HTTP request with exponential backoff for transient errors (502, 503, 429)."""
    backoff = 1.0
    for attempt in range(1, max_attempts + 1):
        try:
            if method.upper() == "POST":
                resp = requests.post(url, headers=headers, params=params, json=json_data, timeout=10)
            elif method.upper() == "PUT":
                resp = requests.put(url, headers=headers, params=params, json=json_data, timeout=10)
            else:
                resp = requests.get(url, headers=headers, params=params, timeout=10)
            
            # If transient error, trigger retry block
            if resp.status_code in [429, 502, 503] and attempt < max_attempts:
                logger.warning(f"Transient HTTP {resp.status_code} on attempt {attempt}. Retrying in {backoff}s...")
                time.sleep(backoff)
                backoff *= 2
                continue
            ...
            if attempt == max_attempts:
                ...
                raise e
            logger.warning(f"Request exception on attempt {attempt}: {e}. Retrying in {backoff}s...")
            time.sleep(backoff)
            backoff *= 2
...
class SheetsClient:
    def __init__(self, spreadsheet_id=None):
        self.spreadsheet_id = spreadsheet_id or os.getenv("SPREADSHEET_ID") or os.getenv("WISMO_SPREADSHEET_ID")

    def get_auth_token(self):
        """Obtains Google Access Token using Application Default Credentials.
        Uses a module-level cache so token refresh only happens on expiry
        ...
        if not _SHARED_CREDENTIALS:
            _SHARED_CREDENTIALS, _ = google.auth.default(scopes=scopes)

        # Refresh token if expired
        if not _SHARED_CREDENTIALS.valid:
            _SHARED_CREDENTIALS.refresh(Request())

        return _SHARED_CREDENTIALS.token
    ...
    def append_log(self, data, range_name="Sheet1!A1"):
        ...
        try:
            ...
            response = execute_with_retry("POST", url, headers, params, payload)
    ...
    def lookup_wismo_mock(
        ...
        try:
            ...
            if not sheet_title:
                logger.info(f"Cache miss: Resolving sheet title for GID {target_gid}...")
            ...
            if not bypass_cache and cached_entry and (now - cached_entry["timestamp"] < CACHE_TTL_SECONDS):
            ...
            po_idx, status_idx, carrier_idx, tracking_idx, details_idx, shipped_date_idx = -1, -1, -1, -1, -1, -1
            ref_idx, zip_idx, phone_idx, email_idx, name_idx, city_idx = -1, -1, -1, -1, -1, -1
            ...
            for idx, cell in enumerate(headers_row):
                if "carrier" in cell:
                    ...
                    details_idx = idx
                elif any(x in cell for x in ["po/reference", "po number", "purchase order"]):
                    ref_idx = idx
                elif any(x in cell for x in ["customer order number", "customer_order_number"]) and ref_idx == -1:
                    ref_idx = idx
                    ...
                    city_idx = idx
                elif "contact person" in cell or "contact_person" in cell or "customer name" in cell:
                    name_idx = idx
            ...
            for row in rows[1:]:
                ...
                if target_po:
                    if ref_idx != -1 and len(row) > ref_idx:
                        ...
                        if val_norm == target_po or (len(target_po) >= 5 and val_norm.endswith(target_po)):
                            match_found = True
                ...
                # Case B: Check Phone column if phone number is given
                    ...
                    if val_norm == target_phone or (len(val_norm) >= 7 and target_phone.endswith(val_norm)) or (len(target_phone) >= 7 and val_norm.endswith(target_phone)):
                        match_found = True
                
                # Case C: Check ZIP + Last Name direct query if no phone/PO is provided
                elif target_zip and target_last_name and not target_phone and not target_po:
                    record_zip = normalize(row[zip_idx])[:5] if (zip_idx != -1 and len(row) > zip_idx) else ""
                    record_name = str(row[name_idx]).strip().lower() if (name_idx != -1 and len(row) > name_idx) else ""
                    record_last_name = normalize(record_name.split()[-1]) if record_name else ""
                    if record_zip == target_zip and record_last_name == target_last_name:
                        match_found = True
            ...
            def format_verified_row(row):
                return {
                    "found": True,
                    "verified": True,
                    "status": str(row[status_idx]).strip() if (status_idx != -1 and len(row) > status_idx) else "Pending",
                    "carrier": str(row[carrier_idx]).strip() if (carrier_idx != -1 and len(row) > carrier_idx) else "FedEx",
                    "tracking_number": str(row[tracking_idx]).strip() if (tracking_idx != -1 and len(row) > tracking_idx) else "N/A",
                    "shipped_date": str(row[shipped_date_idx]).strip() if (shipped_date_idx != -1 and len(row) > shipped_date_idx) else "",
                    "details": str(row[details_idx]).strip() if (details_idx != -1 and len(row) > details_idx) else "Order lookup successful."
                }

            # Helper to extract cell details safely
            def get_cell(row, idx, default="N/A"):
                return str(row[idx]).strip() if (idx != -1 and len(row) > idx) else default
            ...
            if len(matched_rows) > 1:
                ...
                if zip_code or last_name:
                    for row in matched_rows:
                        ...
                        if (zip_code and target_zip == record_zip) or (last_name and target_last_name == record_last_name):
            ...
            logger.error(f"Error looking up mock WISMO in sheet {self.spreadsheet_id}: {e}")
            return None
