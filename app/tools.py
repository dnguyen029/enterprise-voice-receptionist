...
def log_lead(
    ...
    Args:
        ...
        product_type: The optional product type mentioned (vanity, tub, faucet, mirror).
...
def wismo_lookup(
    ...
    logger.info(f"wismo_lookup tool called (PO: {purchase_order}, ZIP: {zip_code}, Phone: {phone_number}, Last Name: {last_name})")
    ...
    try:
        ...
        sheets = SheetsClient(spreadsheet_id=mock_sheet_id)
...
def web_fetch_exa(url: str) -> dict:
    ...
    response = requests.post(
        "https://api.exa.ai/contents", json=payload, headers=headers
    )
    response.raise_for_status()
    data = response.json()
